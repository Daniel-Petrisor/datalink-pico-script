...
def read_current_file(current_file_path):
    ...
    with open(current_file_path, 'r') as text_file:
        ...
        dt_file = read_last_update_file(current_file_path)
...
def run_App(config_path):
    ...
    if os.path.exists(config_path):
        ...
        file_path = config.get('Datalink','CurrentFile')
        sqlite_path = config.get('Datalink','DataBasePath')
        json_path = config.get('Datalink','JsonFilePath')
        json_value = config.get('Datalink','SaveJsonData')

        print(f'''
        CurrentFile : {file_path}
        DataBasePath : {sqlite_path}
        JsonFilePath : {json_path}
        SaveJsonData : {json_value}
        ''')



    # Read last update datetime from the current file
    last_dt = read_last_update_file(file_path)
    ...
    while True:
        ...
        new_dt = read_last_update_file(file_path) # Read last update file ( datetime )
        ...
        if new_dt > last_dt:
            ...
            current_file = read_current_file(file_path) # Read current file from logger directory
            stored_data(current_file, sqlite_path, json_path, json_value) # Stored data in sqlite3 and json
